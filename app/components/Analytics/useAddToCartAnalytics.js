...
export function useAddToCartAnalytics({pageAnalytics, DEBUG}) {
  const {id, lines = [], status, totalQuantity = 0} = useCart();
  const cartLines = lines;

  const [mounted, setMounted] = useState(false);
  const [previousCartCount, setPreviousCartCount] = useState(0);
  const [previousCartLines, setPreviousCartLines] = useState(undefined);
  const [previousCartLinesMap, setPreviousCartLinesMap] = useState(undefined);
  ...
  useEffect(() => {
    ...
    const cartItemsMap = cartLines.reduce((acc, line) => {
