...
export function useDataLayerCart({
  ...
  const [previousCartLines, setPreviousCartLines] = useState(null);
  const [previousCartLinesMap, setPreviousCartLinesMap] = useState(null);

  const addToCartEvent = useCallback(
    ({cart: _cart, lines, userProperties: _userProperties}) => {
  ...
  useEffect(() => {
    ...
    const cartLinesMap = cartLines?.reduce((acc, line) => {
    ...
    const isAddedLines = [];
    const isIncreasedLines = [];
    const isRemovedLines = [];
    const isDecreasedLines = [];
    ...
    if (totalQuantity > previousCartCount) {
      ...
      previousCartLines.forEach((prevLine, index) => {
        const variantId = prevLine.merchandise?.id;
        if (!variantId) return;

        const currentLine = cartLinesMap?.[variantId]?.find((line) => {
          const hasSameSellingPlanSelection =
