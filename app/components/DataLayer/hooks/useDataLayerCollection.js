...
export function useDataLayerCollection({
  ...
  const collectionHandleRef = useRef(undefined);
  ...
  useEffect(() => {
    emitter?.on('VIEW_COLLECTION_PAGE', (products) => {
      setCollectionProducts(products);
    });
    emitter?.on('CLICK_COLLECTION_ITEM', (variant) => {
      setClickedCollectionItem(variant);
    });
    return () => {
      emitter?.off('VIEW_COLLECTION_PAGE', (products) => {
        setCollectionProducts(products);
      });
      emitter?.off('CLICK_COLLECTION_ITEM', (variant) => {
        setClickedCollectionItem(variant);
      });
    };
  }, []);
