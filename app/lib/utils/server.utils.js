...
export const getShop = async (context) => {
...
export const getSiteSettings = async (context) => {
  return await context.pack.query(SITE_SETTINGS_QUERY, {
    cache: context.storefront.CacheShort(),
  });
};

export const getProductGroupings = async (context) => {
...
export const getAccountSeo = async (context, accountTitle) => {
...
export const getEnvs = async (context) => {
    ...
    activeFilterValues = [...searchParams.entries()].reduce(
      (acc, [_paramKey, _paramValue]) => {
        ...
        if (paramKey === PRICE_FILTER_ID) {
          ...
          try {
            defaultPrice = JSON.parse(value.input);
          } catch (error) {}
        ...
        paramValues.forEach((paramValue) => {
          ...
          const value = filter.values.find(
            (filterValue) => filterValue.id === paramValueId,
          );
          if (value) {
            try {
              acc.push({
                ...value,
                parsedInput: JSON.parse(value.input),
                filter: {id: filter.id, label: filter.label, type: filter.type},
              });
            } catch (error) {}
          }
...
export const getMetafields = async (context, {handle, metafieldQueries}) => {
  ...
  const metafields = Object.entries({...product}).reduce((acc, entry) => {
